...
# http://127.0.0.1:5000/api/v1.0/image_code/896526c9-30f3-4d0a-bffa-456251ef5bcb
# GET 127.0.0.1/api/v1.0/image_code/<image_code_id>
@api.route('/image_code/<image_code_id>')
def get_image_code(image_code_id):
    """
    生成图片验证码
    :param image_code_id: 前端生成的全局唯一验证码编号
    :return: 正常:验证码图片  异常：返回json
    """
    # 校验数据

    # 业务逻辑
    # 生成图片验证码,将编号和验证码放入redis中,设置过期时间
    name, text, image_data = captcha.generate_captcha()
    try:
        redis_store.setex('image_code_%s' % image_code_id, constants.IMAGE_CODE_REDIS_EXPIRES,
                          text)  # name, time, value
...
# http://127.0.0.1:5000/api/sms_codes/15026809893?image_code=vc6e&image_code_id=c36489c5-2db9-468e-8a6c-5e75beb4f2cf
...
def get_sms_code(mobile):
    ...
    try:
        ...
        real_image_code = real_image_code.decode('utf8')
    ...
    # 删除redis中的图片验证码，防止用户使用同一个图片验证码验证多次来撞库
    try:
