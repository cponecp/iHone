...
from ihome.models import Area, Facility, House,HouseImage,Order,User
import json
...
def get_area_info():
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg="数据库信息异常")

    resp_li = []
    for area in area_li:
        resp_li.append(area.to_dict())

    area_dic = dict(errno=RET.OK, errmsg="查询成功", data=resp_li)
    area_json = json.dumps(area_dic)

    # 2.1 保存到redis中
    try:
        redis_store.setex('area_info', constants.AREA_INFO_REDIS_CACHE_EXPIRES, area_json)
    except Exception as e:
        current_app.logger.error(e)
